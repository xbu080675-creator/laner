...
enum class DraftHudModule(val key: String, val label: String) {
    ...
    LEFT_PICK("left_pick", "左侧选择"),
    RIGHT_PICK("right_pick", "右侧选择"),
    MATCHUP("matchup", "对位信息"),
    WATERMARK("watermark", "证据标识"),
}
...
/** Android persistence for user-owned Draft HUD layout only. No match facts are stored here. */
object DraftHudLayoutStore {
    private const val PREFS = "laner_draft_hud_layout_v1"
    ...
    private fun defaultPlacement(module: DraftHudModule, landscape: Boolean): DraftHudPlacement =
        if (landscape) {
            when (module) {
                ...
                DraftHudModule.LEFT_PICK -> DraftHudPlacement(0.17f, 0.19f, 0.78f, 0.92f)
                DraftHudModule.RIGHT_PICK -> DraftHudPlacement(0.83f, 0.19f, 0.78f, 0.92f)
            ...
            when (module) {
                ...
                DraftHudModule.LEFT_PICK -> DraftHudPlacement(0.25f, 0.115f, 0.72f, 0.88f)
                DraftHudModule.RIGHT_PICK -> DraftHudPlacement(0.75f, 0.115f, 0.72f, 0.88f)
