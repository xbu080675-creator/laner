...
internal class TacticalHudOverlayView(context: Context) : FrameLayout(context) {
    ...
    private fun renderEvidence(rows: List<TacticalHudEvidence>, accentColor: Int) {
        ...
        rows.take(4).forEachIndexed { index, row ->
            ...
            chip.addView(text(8f, true).apply { this.text = row.label }, LinearLayout.LayoutParams(0, LayoutParams.WRAP_CONTENT, 1f))
    ...
    private fun renderPlayers(rows: List<TacticalHudPlayer>, accentColor: Int) {
        ...
        rows.take(3).forEachIndexed { index, row ->
            val card = panel().apply {
                ...
                addView(View(context).apply { setBackgroundColor(accentColor) }, LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, dp(2)).apply { topMargin = dp(4) })
            }
    ...
    private fun marginTop(value: Int) = LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT).apply { topMargin = dp(value) }
