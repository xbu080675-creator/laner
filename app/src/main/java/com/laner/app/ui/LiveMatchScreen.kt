...
fun LiveMatchScreen(
    ...
    onWatchPresentationChange: (WatchHubPresentation) -> Unit = {},
    ...
    LaunchedEffect(state) {
        val ready = state as? LiveScreenState.Ready
        onWatchPresentationChange(
            if (ready == null) {
                WatchHubPresentation.Idle
            } else {
                WatchHubPresentationMapper.from(ready.match, ready.resolution.state.lifecycle)
            },
        )
    }
