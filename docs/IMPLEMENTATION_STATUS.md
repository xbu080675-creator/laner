...
- Project phase: `M0 / Migration Foundation`
...
Laner 已经进入真实代码开发，不再是纯文档仓库。目前完成的是**迁移地基**，不是完整功能迁移。
...
- GitHub Actions Core Test + Android Compile Gate。

当前 UI 不展示假比赛数据；旧功能只有在 `docs/FEATURE_BASELINE.md` 对应条目通过真实迁移验收后才可标记 DONE。

## Active Gate

`LNR-004` 只有在 GitHub CI 实际通过后才允许从 `TESTING` 切换为 `DONE`。

后续立即进入 `M1` 第一批真实功能迁移：Global Competition Catalog / Schedule / PRE data。
