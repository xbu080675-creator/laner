...
- Code/test review head: `098d6a6896fd443cd39340f8094c7511251f569d`
- Authority-doc sweep head before this record update: `bd0fcb25ad347ed3fdd76bad66d200fa9d9100eb`
- Current status: `PRE-MERGE REMEDIATION PASS / DELIVERY INCOMPLETE UNTIL MERGE + MAIN VERIFY`
...
- `docs/ARCHITECTURE.md`：从错误的 `DRAFT / M0 / 当前无业务实现` 修正为 `ACTIVE / M1 / FROZEN BASELINE`，同时把“待旧工程审计后决定”的过期尾注改为基于当前仓库事实的后续技术选型规则；
- `docs/PROJECT_SCOPE.md`：从错误的 `M0 / LNR-001 / 业务尚未开始 / 基线待提取` 修正为当前 M1、Feature Baseline DONE 和 LNR-020 合规整改门禁；旧 M0 的 blanket N/A 也改为 M1 的真实适用说明；
- `app/README.md` / `core/application/README.md`：同步真实架构与故障定位链。

全仓状态关键词扫尾时，剩余“当前无业务实现 / M0”命中属于历史不可变开发记录或阶段名，不再修改历史来伪造当时状态。

这些现行状态文件当前仍如实写“整改中”，不会在 PR #12 merge 前提前写成 CLOSED。
...
### 6.1 代码/测试 exact-head Gate

代码与主要状态文档 review head：`098d6a6896fd443cd39340f8094c7511251f569d`
...
- workflow head SHA 明确为 `098d6a…`；artifact name 使用 GitHub PR merge-ref SHA，不冒充 branch head。

在该 Gate 后又追加了 `ARCHITECTURE.md / PROJECT_SCOPE.md` 状态同步和本整改记录更新；这些提交不修改生产代码，但根据“exact-head 才是最终证据”规则，当前 PR 最终 head 仍必须重新通过完整 Gate 后才允许 merge。
...
从原 `967e112…` 到 authority-doc sweep head `bd0fcb25…` 共 21 个文件，均属于事故记录、Current LIVE Context、Overlay 拆分、测试、架构/范围与状态文档；没有 Tactical HUD / Draft Provider / Domain 赛事事实实现文件。
...
代码实现以 `098d6a…`、现行权威文档扫尾以 `bd0fcb25…` 为复审事实，逐项检查：
...
| `INC-020-03` 状态漂移 | REMEDIATED PRE-MERGE | IMPLEMENTATION_STATUS / DEVELOPMENT_PLAN / root README / ARCHITECTURE / PROJECT_SCOPE / module README 已同步真实当前状态 |
...
- 权威状态文档不再保留“当前 M0 / 业务未开始 / 下一步 LNR-001”的失真现状；
- 历史开发记录中的 M0/N/A 事实保持不变，没有为了现在的状态回写历史。

**Post-change Compliance Review（pre-merge code/doc scope）：PASS。**

该 PASS 不等价于任务最终 CLOSED：本记录更新会形成新的 docs-only head，仍需 exact-head Gate；随后必须 PR #12 merge，并对 merge 后 main 再做状态落账与 Gate 复核。
...
2. **Baseline branch / start commit / end commit**：branch `fix/lnr-020-constitution-remediation`；start `1b8c6b183ece4b64a4f9ecd8a8c097034d5e37bc`；code/test review head `098d6a6896fd443cd39340f8094c7511251f569d`；authority-doc sweep head `bd0fcb25ad347ed3fdd76bad66d200fa9d9100eb`；最终 end/merge commit 待 post-merge closeout。
3. **Constitution Preflight**：`PASS`；完整读取项见第 2 节；没有把原 LNR-020 事后补写成 PASS。
4. **涉及模块**：`:core:application`, `:app`, root/module README, architecture/scope, docs/development/status/troubleshooting。
5. **强相关条款**：§0.3, §0.4, §1, §1.1, §1.2, §2.2, §3.2, §3.3, §3.4, §8, §9, §11, §13.2, §13.3, §14, §15, L-3, L-5, L-6, L-8。
6. **精确文件增删改（相对原 LNR-020 main `967e112…`，不含本记录自身的最终更新提交）**：新增 `app/src/main/java/com/laner/app/overlay/DraftHudWindowController.kt`, `OverlayWindowHost.kt`, `RiftScreenWindowController.kt`, `app/src/test/java/com/laner/app/overlay/OverlayWindowOperationTest.kt`, `core/application/src/main/kotlin/com/laner/core/application/LiveMatchContextService.kt`, `core/application/src/test/kotlin/com/laner/core/application/LiveMatchContextServiceTest.kt`, `docs/development/2026/2026-09-13_LNR-020_constitution-remediation.md`, `docs/development/2026/2026-09-13_LNR-020_违宪事故记录.md`；修改 `README.md`, `app/README.md`, `app/src/main/java/com/laner/app/LanerAppGraph.kt`, `MainActivity.kt`, `overlay/RiftScreenOverlayService.kt`, `ui/LanerRoot.kt`, `ui/LiveMatchScreen.kt`, `core/application/README.md`, `docs/ARCHITECTURE.md`, `docs/PROJECT_SCOPE.md`, `docs/DEVELOPMENT_PLAN.md`, `docs/IMPLEMENTATION_STATUS.md`, `docs/TROUBLESHOOTING.md`；删除：无。
7. **实现与设计原因**：只修复 `INC-LNR-020-001` 7 项确认偏离及同类状态漂移；不扩功能；详见第 3~4 节。
8. **测试**：正向 PASS；负向 PASS；边界 PASS；异常诊断 PASS；非目标 PASS（FEATURE_BASELINE 状态不变）；历史回归 PASS；Architecture PASS；Integration/Device `WAITING EXTERNAL TEST`。代码自动证据 run `34707759041`；最终 docs-only exact-head Gate 待本提交后执行。
9. **脚本验证**：无本任务专用迁移脚本，`N/A`；Architecture/Gradle workflow run `34707759041` 全 Gate PASS；最终 exact-head 仍需 PASS。
...
11. **影响范围**：Current LIVE Query、Compose LIVE 依赖、RiftScreen/Draft HUD Window orchestration、架构/范围/状态/故障文档；不改变赛事 Domain 事实定义、Provider payload contract 或 Feature Baseline 功能状态。
...
15. **Commit / Push / PR / Release**：PR #12 `DRAFT`；code/test review head `098d6a…`；run `34707759041` PASS；artifact `10302302373`, digest `sha256:7eea41054c3b9525913b3106617ee269b16845f4fd0a042987983f9ec04dbc67`；本记录更新后的 exact-head run 待完成；release `N/A`；merge/main run 待回填。
16. **Post-change Compliance Review**：`PASS` for pre-merge code/doc scope；本记录最终提交的 exact-head Gate 和 merge/main verification 仍是最终交付门禁。
17. **最终结论**：`DELIVERY INCOMPLETE`。7 项偏离及审计中发现的同类现行状态漂移在 pre-merge review 范围已整改通过，但在 PR #12 exact-head Gate、merge、merge 后 main 状态落账和 main Gate 完成前，不得标记事故 CLOSED，也不得进入 Block 2。
